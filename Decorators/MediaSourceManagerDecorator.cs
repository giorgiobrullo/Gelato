...
) : IMediaSourceManager
{
    ...
    )
    {
        ...
        if (sources.Count > 1)
        {
            ...
            sources = sources
                .Where(k => k.Path is null || !k.Path.StartsWith("gelato", StringComparison.OrdinalIgnoreCase))
                .Where(k => k.Path is null || !k.Path.StartsWith("stremio", StringComparison.OrdinalIgnoreCase))
                .ToList();
        }
